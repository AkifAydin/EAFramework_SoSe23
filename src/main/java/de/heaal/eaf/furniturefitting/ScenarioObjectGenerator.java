...
public enum ScenarioObjectGenerator {
    ...
    private static boolean SCENARIO_ONE = false;

    private static boolean SCENARIO_TWO = true;
    ...
    public MovableObject getNewMovableObject() {
        if (SCENARIO_ONE) {
            return new MovableObject(getxPointsOBJ_ONE, getyPointsOBJ_ONE, new Point2D.Double(910, 400));
        } else if (SCENARIO_TWO) {
            //second room
            return new MovableObject(getxPointsOBJ_TWO, getyPointsOBJ_TWO, new Point2D.Double(1300, 400));
        }
        return null;
    }
